...
from .io import BinaryDecoder, ReadError
...
from ._types cimport int32, long64
from .const import (
...
def _iter_avro_records(decoder, header, codec, writer_schema, reader_schema):
    for block in _iter_avro_blocks(decoder, header, codec, writer_schema, reader_schema):
        for record in block:
            yield record
...
def _iter_avro_blocks(decoder, header, codec, writer_schema, reader_schema):
    ...
    while True:
        ...
        try:
            num_block_records = read_long(decoder)
